...
# Your application will contain exactly one instance of a QApplication object (or derived subclass)
class SayHelloApp(qtw.QApplication):
    '''
    Contains a VerticalHelloBox to prompt the user for their name and say hello!
    '''
    def __init__(self):
        # Call the instructor for the parent class
        # The one required argument is usually sys.argv, which are command line parameters
        # We don't need that, hence the empty list
        super().__init__([])
...
class WindowWithVerticalSlots(qtw.qWidget):
    '''
    A window with a title and an epmty vertical container (QVBoxLayout).

    Intended use of this class is to inherit and extend.
    '''
    def __init__(self, title: str):
        super().__init__()

        # Set the window title
        self.setWindowTile(title)
...
class VerticalHelloBox(WindowWithVerticalSlots):
    '''
    Contains a textbox and a button the user can press wchich opens up another window for them to enter their name
    '''
    def __init__(self):
    ...
    def configure(self) -> None:
        ...
        # Bind the button to a function, so that when the button is pressed the function is called
    ...
    def hello_button_clicked(self) -> None:
        ...
        if name_getter.exec_() == qtw.QDialog.Accepted:
            ...
            self.greeting_box.getText(f'Hello {name_getter.get_text()})')
        return None
    
class InputPopup(qtw.QDialog):
    '''
    A popup window with a text box and an OK button to allow the user to enter freeform text
    '''
    ...
    def __init__(self, title: str):
        ...
        # Place to enter your text
        self.text_entry = qtw.QLineEdit(self)

        # Ok button to click
        self.ok_button = qtw.QPushButton('Ok', self)

        # When the button is clicked, it calls the accept() method of the QDialog, 
        # which lets the application know that the interaction with the dialog is complete
        self.ok_button.clicked.connect(self.accept)

        # Create a vertical layout and add the widgets
        self.my_layout = qtw.QBoxLayout(self)
